...
void getcmd(const Block *block, int j, char *output)
{
       ...
       if (i != 0) {
              ...
              i = tempstatus[i-1] == '\n' ? i-1 : i;
              if (delim[0] != '\0' && j != LENGTH(blocks)-1) {
...
int getstatus(char *str, char *last)
{
       strcpy(last, str);
       str[0] = '\0';
       strcat(str,startdelim);
       for (unsigned int i = 0; i < LENGTH(blocks); i++)
              strcat(str, statusbar[i]);
       strcat(str,enddelim);
       str[strlen(str)-strlen(enddelim)] = '\0';
